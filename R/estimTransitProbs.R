...
#' @param max_num_lag maximum number of lagged CpGs for estimation of
...
.computeProb1Unit <- function(df, max_dist_bp, buffer_bp, max_num_lag){
  ...
  .computeLag <- function(lag) {
    lag_df <- df %>%
      group_by(chr) %>%
      mutate(state_lag = dplyr::lag(state, lag = lag),
             dist_bp = c(rep(NA, lag), diff(pos, lag = lag))) %>%
      ungroup() %>%
      select(state, state_lag, dist_bp) %>%
      dplyr::filter(dist_bp <= max_dist_bp + buffer_bp)
    return(lag_df)
  }

  smr_df <- do.call(rbind, map(1:max_num_lag, .computeLag)) %>%
  # smr_df <- df %>%
  #   group_by(chr) %>%
...
.estimTransitProbsFromSummary <- function(smr_units, max_dist_bp, buffer_bp, degree, span, ...){
  ...
  # smoothed_probs <- with(
  #   train_data %>% dplyr::filter(var_00!=0, var_01!=0, var_10!=0, var_11!=0),
